...
#define COLOR_OFFLINE 0xFC00 // warning orange, tweak to taste
...
void display_init_log(void)
{
    ...
    while (*str)
    {
        ...
        {
            ili9341_draw_char(cursor_x, cursor_y, *str, COLOR_BLUE, 0xffff, 2);
            cursor_x += char_width + x_spacing;
        }
        else
        {
    ...
    ili9341_fill_area(4, 310, 232, 4, COLOR_BLUE); // bottom line
}
...
void update_tank_gauge(uint8_t percent)
{
    ...
    if (fill_h > 0)
    {
        ...
        ili9341_fill_area(TANK_X, fill_y, TANK_W, fill_h, COLOR_BLUE);
    }
...
// void draw_tx_offline(void)
// {
//     // paint the whole tank interior one neutral color — no water/empty split,
//     // since the level shown would be stale, not current
//     ili9341_fill_area(TANK_X, TANK_Y, TANK_W, TANK_H, COLOR_OFFLINE); // orange colour

//     const char *msg = "TX OFFLINE";
//     uint8_t len = 0;
//     while (msg[len] != '\0')
//     {
//         len++;
...
//     uint16_t text_w = (len * 12) - 2; // 10px char width at scale 1 + 2px spacing per char
//     uint16_t text_h = 14;             // 14px char height at scale 1

//     uint16_t label_x = TANK_X + (TANK_W - text_w) / 2;
//     uint16_t label_y = TANK_Y + (TANK_H - text_h) / 2;

//     ili9341_draw_string(label_x, label_y, msg, COLOR_BLACK, COLOR_OFFLINE, 1);
// }
...
void draw_tx_offline(void)
{
    ...
    uint16_t text_w1 = (len1 * 22) - 2; // 20px char width at scale 1 + 2px spacing per char
    uint16_t text_w2 = (len2 * 22) - 2; // 20px char width at scale 1 + 2px spacing per char
    uint16_t text_h = 28;               // 28px char height at scale 1

    uint16_t label1_x = TANK_X + (TANK_W - text_w1) / 2;
    uint16_t label2_x = TANK_X + (TANK_W - text_w2) / 2;

    uint16_t label1_y = TANK_Y + (TANK_H - text_h - text_h - 2) / 2;
    uint16_t label2_y = TANK_Y + (TANK_H + 2) / 2;

    ili9341_draw_string(label1_x, label1_y, msg1, COLOR_BLACK, COLOR_OFFLINE, 2);
    ili9341_draw_string(label2_x, label2_y, msg2, COLOR_BLACK, COLOR_OFFLINE, 2);
}